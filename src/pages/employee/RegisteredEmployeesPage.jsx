...
function Tile({ label, value, icon, accent }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: '10px 14px',
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color: accent || '#0f172a', lineHeight: 1.3 }}>
        {value}
      </div>
    </div>
  );
...
function SecHead({ title }) {
  return (
    <div style={{ gridColumn: '1 / -1', display: 'flex', alignItems: 'center',
      ...
      <div style={{ fontSize: 11, fontWeight: 800, color: '#c0392b',
                    letterSpacing: '.4px', textTransform: 'uppercase' }}>{title}</div>
      <div style={{ flex: 1, height: 2,
                    background: 'linear-gradient(90deg,#fee2e2,transparent)', borderRadius: 2 }} />
    </div>
  );
}
...
function RegisteredEmployeesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Client-side active/inactive filter (backend doesn't support it yet)
    ...
    <div>
      ...
      <PageCard>
                  ...
                  <tr>
                    <td colSpan={10} style={{ padding: 40, textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>
                      {hasFilters
                        ...
                        : 'No employees registered via the mobile app yet.'}
                    </td>
                  </tr>
                ) : employees.map(e => (
                  <tr key={e.id}
                    ...
                    <td style={TD} onClick={ev => ev.stopPropagation()}>
                      <button onClick={() => setViewEmp(e)} title="View Details"
        ...
        {!loading && employees.length > 0 && (
          <div style={{ marginTop: 10, fontSize: 11.5, color: '#94a3b8', textAlign: 'right' }}>
          ...
          return (
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20,
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 700,
                                textTransform: 'uppercase', letterSpacing: '.5px' }}>Employee ID</div>
                  <div style={{ fontFamily: 'monospace', fontSize: 14, fontWeight: 800,
                                color: '#3b82f6', marginTop: 2 }}>{e.employeeId}</div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                ...
                <Tile label="Account Status"
                  value={e.isActive ? 'Active' : 'Inactive'}
                  accent={e.isActive ? '#16a34a' : '#dc2626'} />
              </div>
            </div>
          );
        })()}
