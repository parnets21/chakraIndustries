...
const ACCENT_COLOR = '#16a34a'; const ACCENT_BG = '#f0fdf4'; const ACCENT_BORDER = '#bbf7d0';

export default function SingleProductInvoices({
    ...
    <div style={{ background: '#fff', borderRadius: 18, border: `1.5px solid ${ACCENT_BORDER}`, boxShadow: '0 2px 12px rgba(15,23,42,0.06)', overflow: 'hidden', marginBottom: 24 }}>
      ...
      <div style={{ padding: '14px 20px', borderBottom: `1px solid ${ACCENT_BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', background: ACCENT_BG }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: TEXT_DARK }}>Single Product Invoices</div>
            <div style={{ fontSize: 11, color: TEXT_MID, marginTop: 1 }}>Customers with one product per order</div>
          </div>
      ...
      {/* ── Table ── */}
      {invoices.length === 0 ? (
        <div style={{ padding: '36px 20px', textAlign: 'center', color: TEXT_LIGHT, fontSize: 13 }}>No single-product invoices yet. Upload an Excel file.</div>
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                {['#', 'Invoice No', 'Unique ID', 'PO Number', 'PO Date', 'Ship To', 'City', 'State', 'Product', 'Brand', 'Qty', 'UOM', 'Dispatch Date', 'AWB', 'Courier', 'Actions'].map(h => (
              ...
              {invoices.map((inv, i) => {
                ...
                return (
                  <tr key={inv._id} style={{ borderBottom: '1px solid #f1f5f9', background: isManualStock ? '#fffbeb' : (i % 2 === 0 ? '#f8fafc' : '#fff') }}>
                    ...
                    <td style={{ padding: '9px 12px', color: TEXT_DARK, fontWeight: 700, textAlign: 'center' }}>{item.qty ?? '—'}</td>
