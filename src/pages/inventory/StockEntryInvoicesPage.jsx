...
const fmt  = (n) => `Γé╣${(Number(n) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`;
const fmtD = (d) => {
...
// ΓöÇΓöÇ printable invoice HTML ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function buildHTML(inv) {
  ...
  const rows   = items.map((it, i) => {
    ...
    return `<tr>
      ...
      <td style="text-align:center">${esc(it.hsn || 'ΓÇö')}</td>
...
<title>Manual Stock Invoice ΓÇö ${esc(inv.invoiceNo)}</title>
<style>
...
<div class="party-row">
  ...
  <div class="party-cell">
    ...
    <div class="party-detail">${esc(inv.notes || 'ΓÇö')}</div>
  </div>
</div>
...
export default function StockEntryInvoicesPage() {
  ...
  const kpis = [
    ...
    { label: 'Total Value',    value: `Γé╣${(totalVal / 1000).toFixed(1)}K`, icon: <MdAttachMoney size={18} />, color: '#7c3aed', color2: '#8b5cf6', glow: 'rgba(124,58,237,0.2)' },
  ];
  ...
  return (
    <div>
      <PageHeader
        ...
        breadcrumb="Inventory ΓÇ║ Stock Entry Invoices"
        action={
          <button
      ...
      <PageCard>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
            ...
            <input
              ...
              placeholder="Search invoice no., SKU, item nameΓÇª"
        ...
        {/* Table */}
        {loading ? (
          <div style={{ padding: '40px 0', textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>Loading invoicesΓÇª</div>
        ) : filtered.length === 0 ? (
                      ...
                      <td style={{ padding: '11px 12px' }}>
                        <div style={{ display: 'flex', gap: 5 }}>
                          ...
                          <button onClick={() => handleDownload(inv)} disabled={downloading === inv._id} title="PDF"
                            ...
                            <MdDownload size={13} /> {downloading === inv._id ? 'ΓÇª' : 'PDF'}
                          </button>
                        </div>
                      </td>
                    </tr>
      ...
      {/* ΓöÇΓöÇ View / Approve Modal ΓöÇΓöÇ */}
      <Modal
        open={!!viewInv}
        onClose={() => setViewInv(null)}
        title={`Stock Entry Invoice ΓÇö ${viewInv?.invoiceNo}`}
        size="xl"
        footer={
          <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => viewInv && handleDownload(viewInv)} disabled={downloading === viewInv?._id}
                ...
                <MdDownload size={15} /> {downloading === viewInv?._id ? 'GeneratingΓÇª' : 'PDF'}
              </button>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 16 }}>
              {[
                ...
                { label: 'Item', value: viewInv.items?.[0]?.description || 'ΓÇö' },
                { label: 'Qty', value: `${viewInv.items?.[0]?.qty ?? 'ΓÇö'} ${viewInv.items?.[0]?.unit || ''}` },
