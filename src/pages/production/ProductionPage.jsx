  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"flex-end", gap:10, marginBottom:20, flexWrap:"wrap" }}>
        {activeTab === 0 && <><button onClick={() => setShowBOMModal(true)} style={primaryBtn}>+ New BOM</button>{selectedBOM && <button onClick={() => setShowCompModal(true)} style={outlineBtn}>+ Add Component</button>}</>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
            <div className="flex justify-between items-center mb-3">
              ...
              {selectedBOM && <button onClick={() => setShowCompModal(true)} style={{ ...primaryBtn, padding:"5px 12px", fontSize:12 }}>+ Add</button>}
            </div>
      ...
      {/* New BOM Modal */}
      <Modal isOpen={showBOMModal} onClose={() => setShowBOMModal(false)} title="Create New BOM">
      ...
      {/* Add Component Modal */}
      <Modal isOpen={showCompModal} onClose={() => setShowCompModal(false)} title={`Add Component — ${selectedBOM?.product || ""}`}>
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
      ...
      {/* Add Alternate Modal */}
      <Modal isOpen={!!showAltModal} onClose={() => setShowAltModal(null)} title="Add Alternate Item">
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
      ...
      <Modal isOpen={!!showProgressModal} onClose={() => { setShowProgressModal(null); setProgressVal(''); }} title={`Update Progress — ${showProgressModal?.woId || ""}`}>
        <div className="flex flex-col gap-3">
      ...
      {/* QC Modal */}
      <Modal isOpen={!!showQCModal} onClose={() => { setShowQCModal(null); setQcForm(EMPTY_QC); }} title={`Record QC — ${showQCModal?.woId || ""}`}>
        <div className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3">
      ...
      {/* Approve BOM Modal */}
      <Modal isOpen={!!showApproveModal} onClose={() => setShowApproveModal(null)} title={`Review BOM — ${showApproveModal?.product || ""}`}>
        <div className="flex flex-col gap-3">
      ...
      {/* Run MRP Modal */}
      <Modal isOpen={showMRPModal} onClose={() => setShowMRPModal(false)} title="Run MRP">
        <div className="flex flex-col gap-3">
      ...
      {/* BOM Explode Modal */}
      <Modal isOpen={!!showExplodeModal} onClose={() => { setShowExplodeModal(null); setExplodeData([]); }} title={`BOM Explosion — ${showExplodeModal?.product || ""}`}>
        <div style={{ overflowX:"auto" }}>
      ...
      <Modal isOpen={!!showConsumptionModal} onClose={() => setShowConsumptionModal(null)} title={`Material Consumption — ${showConsumptionModal?.woId || ""}`}>
        <div style={{ overflowX:"auto" }}>
